...
def train(args):
    ...
    ckpt_path = CHECKPOINTS_DIR / "vqvae_mnist.pt"
    resume_path = CHECKPOINTS_DIR / "vqvae_mnist_resume.pt"
    start_epoch = 1
    ...
    log_csv = LOGS_DIR / "vqvae_train_log.csv"
    if not (args.resume and log_csv.exists()):
        log_csv.write_text("epoch,total_loss,bce,commitment,perplexity\n")

    for epoch in range(start_epoch, start_epoch + args.epochs):
        model.train()
        tot = bce_tot = commit_tot = perp_tot = 0.0
        for x, _ in loader:
            x = x.to(device)
            opt.zero_grad()
            recon, commitment, _, perplexity = model(x)
            loss, bce, commit = vqvae_loss(recon, x, commitment)
            loss.backward()
            opt.step()
            tot += loss.item()
            bce_tot += bce.item()
            commit_tot += commit.item()
            perp_tot += perplexity.item()

        n = len(dataset)
        n_batches = len(loader)
        print(f"E{epoch:02d} | Loss/img {tot/n:.2f} | BCE/img {bce_tot/n:.2f} | "
              f"Commit/img {commit_tot/n:.4f} | Perplejidad media {perp_tot/n_batches:.1f}"
              f" / {args.embeddings}")
        with open(log_csv, "a") as f:
            f.write(f"{epoch},{tot},{bce_tot},{commit_tot},{perp_tot/n_batches}\n")
